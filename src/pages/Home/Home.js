...
import { Switch, Carousel, Tabs } from "antd";
import {
  arrHome,
  arrPhoBien,
  ...
  const items = [
    {
      ...
      children: (
        <>
          <div className={`grid ${windowSize.widthWindow <= 426 ? 'grid-cols-2' : 'grid-cols-5'}`}>
  ...
  return (
    <div className="relative">
      <div className={`${windowSize.widthWindow <= 1024 ? 'px-[40px]' : 'px-[80px]'} pb-5`}>
        <div id="header-content" style={{ position: 'fixed', top: '78px', right: '0', left: '0', background: 'white', zIndex: '5', boxShadow: 'none' }} className={scrolled ? 'scrolled' : ''}>
          <div className={`mx-auto max-w-screen-2xl mt-3 ${windowSize.widthWindow <= 1024 ? 'px-[40px]' : 'px-[80px]'}`}>
            <div className="flex justify-between items-center ">
              <div className={`${windowSize.widthWindow <= 426 ? 'w-full' : windowSize.widthWindow <= 768 ? 'w-[50%]' : windowSize.widthWindow <= 1024 ? 'w-[65%]' : 'w-3/4'}`}>
                <div className={`${windowSize.widthWindow <= 426 ? 'h-[70px] grid-cols-4' : windowSize.widthWindow <= 768 ? 'grid-cols-4 h-[78px]' : windowSize.widthWindow <= 1024 ? 'grid-cols-8 h-[78px]' : 'grid-cols-12 h-[78px]'} grid truncate`}>
              ...
              <div className={`${windowSize.widthWindow <= 426 ? 'hidden' : ''}`}>
                <button className="px-4 py-4 border font-medium text-xs rounded-lg">
                  <i className="fa-solid fa-sliders mr-2"></i>
                  Bộ lọc
                </button>
              </div>
              <div className={`flex items-center py-4 px-4 gap-2 rounded-lg border ${windowSize.widthWindow <= 426 ? 'hidden' : ''}`}>
        ...
        <div className={`my-3 grid gap-5 mt-44 ${windowSize.widthWindow <= 426 ? 'grid-cols-1' : windowSize.widthWindow <= 768 ? 'grid-cols-2' : 'grid-cols-4'}`}>
          {arrPhong
